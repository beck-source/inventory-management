...
- **Always document non-obvious logic changes with comments** — when a change has a hidden constraint, subtle invariant, workaround, or behavior that would surprise a reader, leave a brief comment explaining the WHY (not the WHAT). Skip comments on straightforward changes where good naming already makes intent clear.
