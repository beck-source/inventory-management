...
export default {
  ...
  setup() {
    ...
    const { startMonth, endMonth, selectedLocation, selectedCategory, selectedStatus, getCurrentFilters } = useFilters()
    ...
    // Currency symbol derived from currentCurrency, matching the pattern used on other pages
    const currencySymbol = computed(() => currentCurrency.value === 'JPY' ? '¥' : '$')

    // Summary stats derived from raw data refs — no separate state needed
    ...
    watch(
      [startMonth, endMonth, selectedLocation, selectedCategory, selectedStatus],
      () => loadData()
    )
    ...
    const formatMonth = (monthStr) => {
      // Convert YYYY-MM to localised readable format using i18n month keys
    ...
    const getBarHeight = (revenue) => {
      // Scale bar height proportionally against the max revenue in the dataset (max 200px)
