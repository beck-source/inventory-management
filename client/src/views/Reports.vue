    ...
    <div v-else>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="q in quarterlyData" :key="q.quarter">
                ...
                <td>{{ formatCurrency(q.total_revenue, currentCurrency.value) }}</td>
                <td>{{ formatCurrency(q.avg_order_value, currentCurrency.value) }}</td>
                <td>
            ...
            <div v-for="month in monthlyData" :key="month.month" class="bar-wrapper">
              <div class="bar-container">
                <div
                  ...
                  :title="formatCurrency(month.revenue, currentCurrency.value)"
                ></div>
              </div>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>{{ formatCurrency(month.revenue, currentCurrency.value) }}</td>
                <td>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-value">{{ formatCurrency(totalRevenue, currentCurrency.value) }}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">{{ t('reports.stats.avgMonthlyRevenue') }}</div>
          <div class="stat-value">{{ formatCurrency(avgMonthlyRevenue, currentCurrency.value) }}</div>
        </div>
        <div class="stat-card">
...
export default {
  ...
  setup() {
    ...
    const bestQuarter = computed(() => quarterlyData.value.reduce((best, q) => q.total_revenue > (best?.total_revenue ?? 0) ? q : best, null)?.quarter ?? '')

    // Computed max revenue for O(1) bar height lookups
    ...
    const formatMonth = (monthStr) => {
      ...
      return new Date(monthStr + '-01').toLocaleDateString(locale, { year: 'numeric', month: 'short' })
    }
