...
const { t, currentCurrency, dateLocale } = useI18n()
...
// Summary stats derived from API data via computed properties (Bug #12)
...
// Locale-aware month formatting (Bug #7)
const formatMonth = (monthStr) => {
  const date = new Date(monthStr + '-01')
  if (isNaN(date.getTime())) return monthStr
  return date.toLocaleDateString(dateLocale.value, { month: 'short', year: 'numeric' })
}

// Bar height calculation — max 200px (kept as plain function per spec)
...
// Bug #13: use formatCurrency, strip its leading sign char to avoid double-minus
...
// Watch all filter refs and reload (Bug #5)
