      ...
      <div class="card">
        ...
        <div class="table-container">
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(month, i) in monthlyData" :key="month.month">
                <td><strong>{{ formatMonth(month.month) }}</strong></td>
                <td>{{ month.order_count }}</td>
                <td>{{ formatCurrency(month.revenue, selectedCurrency) }}</td>
                <td>
                  <span v-if="i > 0" :class="getChangeClass(month.revenue, monthlyData[i - 1].revenue)">
                    {{ getChangeValue(month.revenue, monthlyData[i - 1].revenue) }}
                  </span>
                  <span v-else>-</span>
                </td>
                <td>
                  <span v-if="i > 0" :class="getChangeClass(month.revenue, monthlyData[i - 1].revenue)">
                    {{ getGrowthRate(month.revenue, monthlyData[i - 1].revenue) }}
                  </span>
                  <span v-else>-</span>
                </td>
              </tr>
...
export default {
  ...
  setup() {
    ...
    const formatMonth = (monthStr) => {
      ...
      return new Date(year, month).toLocaleDateString(locale, { month: 'short', year: 'numeric' })
    }
