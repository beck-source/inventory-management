                ...
                <tr v-for="item in backlogItems" :key="item.id">
                  ...
                  <td>
                    <span
                      ...
                      class="badge info"
                      :title="'PO ' + item.purchase_order_id"
                    >
                      PO Issued
                    </span>
                    <span v-else class="badge warning">Awaiting PO</span>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  setup() {
    ...
    // PurchaseOrderModal was referenced but never imported (Vue warn on every Dashboard mount).
    // Removed in Tier-1 audit cleanup — the Inventory Shortages table now shows a status
    // badge instead of a Create/View PO button. See audit-action-plan.md item #1.
