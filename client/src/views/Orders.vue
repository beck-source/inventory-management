    ...
    <div v-else>
      ...
      <div class="card" v-if="submittedOrders.length > 0">
        <div class="card-header">
          <h3 class="card-title">
            {{ t("orders.submittedTitle") }} ({{ submittedOrders.length }})
          </h3>
        </div>
        <div class="table-container">
          <table>
            <thead>
              <tr>
                <th>{{ t("orders.submittedTable.id") }}</th>
                <th>{{ t("orders.submittedTable.warehouse") }}</th>
                ...
                <th>{{ t("orders.submittedTable.eta") }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="o in submittedOrders" :key="o.id">
                ...
                <td>{{ o.warehouse }}</td>
                <td>
                  {{ t("orders.submittedItems", { count: o.items.length }) }}
                </td>
                <td>
                  <strong
                    >{{ currencySymbol
                    }}{{ o.total_value.toLocaleString() }}</strong
                  >
                </td>
                <td>{{ formatDate(o.submitted_date) }}</td>
                <td>
                  {{
                    t("orders.submittedLeadDays", { days: o.lead_time_days })
                  }}
                </td>
