<template>
  ...
  >
    ...
    <div class="h-16 flex items-center gap-3 px-4 border-b border-white/10">
      <div
        class="w-9 h-9 rounded-lg bg-gradient-to-br from-sia-sky to-sia-blush flex-shrink-0 flex items-center justify-center shadow-inner"
      >
        <span class="text-sia-navy font-extrabold text-sm tracking-tight"
          >S</span
        >
      </div>
      <div v-if="!collapsed" class="overflow-hidden">
        <div
          class="font-bold tracking-tight text-[15px] leading-tight whitespace-nowrap"
        >
          {{ t("nav.companyName") }}
        </div>
        <div class="text-[11px] text-white/50 leading-tight whitespace-nowrap">
          {{ t("nav.subtitle") }}
        </div>
      </div>
    </div>
...
const items = computed(() => [
  ...
  { path: "/restocking", label: t("nav.restocking"), icon: RotateCw },
