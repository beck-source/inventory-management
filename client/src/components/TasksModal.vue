<template>
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="isOpen" class="modal-overlay" @click="closeWithGuard">
        <div
          ref="containerRef"
          v-bind="ariaProps"
        ...
        >
          <div class="modal-header">
            <h3 :id="titleId" class="modal-title">{{ t("tasks.title") }}</h3>
            <button
              class="close-button"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Overlay-click guard: when the user has started entering a new task,
    // require an explicit confirm before discarding it. See audit-action-plan.md item #17.
    const closeWithGuard = () => {
      const dirty = newTask.value.title.trim() || newTask.value.dueDate;
      if (dirty && !window.confirm("Discard your unsaved task?")) return;
      close();
    };
